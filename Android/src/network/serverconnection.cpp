...
void ServerConnection::getImages() {
...
void ServerConnection::handleMessage(const QString &message) {
    ...
    switch (static_cast<NetworkActionTypes>(type)) {
        ...
        case NetworkActionTypes::ALBUM_INFO: {
            QPair<QString, double> data;
            data.first = messageValue.toObject().value("albumName").toString();
            data.second = messageValue.toObject().value("albumSize").toDouble();
            Dispatcher::get().dispatch(new Action(ActionType::RECEIVE_ALBUM_INFO, QVariant::fromValue(data)));
            break;
        }
    }
}
...
void ServerConnection::onAuth(bool auth) {
    if(!auth) {
        ...
        QSharedPointer<Action> action(new Action(ActionType::CLIENT_CONNECTED));
        Dispatcher::get().dispatch(action);
        sendMessage(QJsonDocument(createMessage(static_cast<int>(NetworkActionTypes::ALBUM_INFO), ""))
                                    .toJson(QJsonDocument::Compact));
    } else {
