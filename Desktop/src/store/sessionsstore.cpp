...
void SessionsStore::process(const QSharedPointer<Action> &action) {
    switch (action->getType<ActionType>()) {
        ...
        case ActionType::BEGIN_SEND_ALBUM_INFO: {
            processSendAlbumInfo(action->getData<networkMessage_t>());
            break;
        }
        case ActionType::BEGIN_SEND_IMAGES: {
...
int SessionsStore::getSessionsCount() {
...
void SessionsStore::processSendAlbumInfo(const networkMessage_t &&message) {
    auto it = std::find_if(m_Sessions.begin(), m_Sessions.end(), [=](Session session) {
        return session.getSessionId() == message.clientLink;
    });
    if(it != m_Sessions.end()) {
        ImageConventer conventer;
        QFuture<QPair<QString, int>> future;
        QFutureWatcher<QPair<QString, int>> *watcher = new QFutureWatcher<QPair<QString, int>>();
        future = conventer.getAlbumSize(it->getAlbumPath());
        connect(watcher, &QFutureWatcher<QPair<QString, int>>::finished, [message, watcher]() {
            QPair<QString, int> data = watcher->result();
            networkMessage_t result;
            result.clientLink = message.clientLink;
            result.data = QVariant::fromValue(data);
            watcher->deleteLater();
            Dispatcher::get().dispatch(new Action(ActionType::SEND_ALBUM_INFO, QVariant::fromValue(result)));
        });
        watcher->setFuture(future);
    }
}
