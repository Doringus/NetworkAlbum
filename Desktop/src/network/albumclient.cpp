...
void AlbumClient::sendAlbumInfo(QString albumName, int albumSize) {
    QJsonObject obj;
    obj.insert("albumName", albumName);
    obj.insert("albumSize", (double)albumSize / (1024.0 * 1024.0));
    sendMessage(QJsonDocument(createMessage(static_cast<int>(NetworkActionTypes::ALBUM_INFO), obj))
                .toJson(QJsonDocument::Compact));
}
